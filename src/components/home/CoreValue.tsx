...
function FeatureIcon({ idx }: { idx: number }) {
  ...
  if (idx === 0) return <Video className={common} />;
  if (idx === 1) return <Sparkles className={common} />;
        ...
        <article className="relative overflow-hidden rounded-[34px] bg-[radial-gradient(circle_at_10%_20%,rgba(186,154,92,0.28),transparent_36%),linear-gradient(130deg,rgba(10,18,35,0.98),rgba(15,28,53,0.94))] p-6 md:p-10">
          ...
          <div className="grid gap-6 md:grid-cols-2 md:items-center">
            <div>
              ...
              <h3 className="mt-7 text-sm font-semibold uppercase tracking-wide text-white/88">
                {t.missionTitle}
              </h3>
          ...
          >
            ...
            <div className="lg:col-span-8">
              ...
              >
                <motion.div
                  ...
                  className="pointer-events-none absolute -top-16 -right-16 h-44 w-44 rounded-full bg-[rgb(var(--gold))]/30 blur-3xl"
                />
              ...
              {/* Cards + subtle connector */}
              <div className="relative mt-6">
                {/* connector line for desktop */}
                <div className="pointer-events-none absolute left-0 right-0 top-6 hidden h-px bg-gradient-to-r from-transparent via-[rgb(var(--gold))]/35 to-transparent md:block" />
                <div className="pointer-events-none absolute left-0 right-0 top-6 hidden md:block">
                  <div className="mx-auto flex max-w-[620px] items-center justify-between">
                    {[0, 1, 2].map((i) => (
                      <span
                        key={i}
                        className="relative inline-flex h-2.5 w-2.5 rounded-full bg-[rgb(var(--gold))]/70 shadow-[0_0_18px_rgba(186,154,92,0.55)]"
                      >
                        <span className="absolute inset-0 rounded-full bg-[rgb(var(--gold))]/55 animate-ping" />
                      </span>
                    ))}
                  </div>
                </div>
                  ...
                  {t.pillars.map((x, idx) => (
                    ...
                    >
                      ...
                      <div className="pointer-events-none absolute -inset-20 opacity-0 blur-3xl transition-opacity duration-700 group-hover:opacity-100">
                        <div className="h-56 w-56 rounded-full bg-[rgb(var(--gold))]/22" />
                      </div>
                      ...
                      {x.image ? (
                        <div className="relative mt-2 aspect-[4/3] overflow-hidden rounded-xl bg-black/10">
                          ...
                          />
                          <div className="absolute inset-0 bg-[linear-gradient(to_top,rgba(10,18,35,0.55),transparent_60%)]" />
                        </div>
                      ) : null}

                      <h4 className="mt-3 text-sm font-semibold text-white">{x.t}</h4>
                      <p className="mt-2 text-sm leading-relaxed text-white/78">{x.d}</p>

                      <div className="mt-4 flex items-center gap-2 text-xs text-white/70">
                        <span className="h-1.5 w-1.5 rounded-full bg-[rgb(var(--gold))]" />
                        {lang === "ar" ? "محور أساسي" : "Core pillar"}
                      </div>
                    </motion.div>
                  ))}
                </motion.div>
            ...
            >
              ...
              <div className="pointer-events-none absolute left-6 top-10 hidden items-center gap-2 rounded-full border border-white/12 bg-white/10 px-3 py-2 text-xs text-white/80 backdrop-blur md:flex">
                ...
                {lang === "ar" ? "Workflow موثق" : "Verified workflow"}
              </div>
