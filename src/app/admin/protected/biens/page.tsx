...
export default async function AdminPage({
  ...
  async function deleteProperty(formData: FormData) {
    ...
    revalidatePath("/admin/protected/biens");
  }
  ...
  function withParams(overrides: Record<string, string | number | null>) {
    ...
    return `/admin/protected/biens${str ? `?${str}` : ""}`;
  }
    ...
    <main className="space-y-8">
      <section className="relative overflow-hidden rounded-3xl border border-black/10 bg-white/75 p-6 shadow-sm backdrop-blur md:p-8">
        ...
        <div className="relative flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-extrabold text-[rgb(var(--navy))]">Catalogue des biens</h1>
            <p className="mt-2 max-w-2xl text-sm text-black/65">
              Vue complete des biens avec filtres avances, pagination, edition rapide et controle qualite.
            </p>
      ...
      <section>
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            <h2 className="text-xl font-extrabold text-[rgb(var(--navy))]">Liste des biens</h2>
            <p className="text-sm text-black/60">Filtrez, parcourez et gerez le portefeuille immobilier.</p>
          </div>
