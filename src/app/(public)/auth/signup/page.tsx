...
export default function SignupPage() {
  ...
  async function sendOtp(e?: React.FormEvent) {
    ...
    setStep("otp");
    setMsg("Code envoyé par SMS. Entrez le code pour finaliser.");
  }
  ...
  async function verifyOtp(e: React.FormEvent) {
    ...
    // Optional: store username in auth metadata (simple)
    // If you prefer profiles table, tell me and I'll do it properly.
    if (username.trim()) {
        ...
        <div className="w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-2xl backdrop-blur">
          ...
          <div className="p-7 sm:p-8">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-white/80">Nom d’utilisateur (optionnel)</label>
              <input
                type="text"
                value={username}
                onChange={(e) => setUsername(e.target.value)}
                placeholder="ex: rostorymia_user"
              ...
              />
              <p className="text-xs text-white/45">Vous pouvez le compléter plus tard.</p>
            </div>
            ...
            <div className="mt-2 text-center text-sm text-white/60">
              ...
              <Link href="/agency/signup" className="font-semibold text-emerald-300 hover:text-emerald-200">
                Inscription agence
              </Link>
            </div>
          </div>
